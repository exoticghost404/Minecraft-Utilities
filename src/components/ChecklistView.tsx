...
import { Search, RotateCcw, LayoutGrid, CheckCircle2, ArrowLeft, Layers, ToggleLeft, ToggleRight } from 'lucide-react';
...
export const ChecklistView: React.FC<ChecklistViewProps> = ({ onBack, initialSearch = '' }) => {
  ...
  const [includeSmithingProgression, setIncludeSmithingProgression] = useState(true);
  ...
  useEffect(() => {
    ...
    if (savedSettings) {
        try {
            ...
            if (settings.includeSmithing !== undefined) setIncludeSmithingProgression(settings.includeSmithing);
        } catch (e) {}
    }
    setIsLoaded(true);
  }, []);
  ...
  useEffect(() => {
    if (isLoaded) {
      ...
      localStorage.setItem('mc_checklist_settings', JSON.stringify({ includeSmithing: includeSmithingProgression }));
    }
  }, [checkedState, includeSmithingProgression, isLoaded]);
  ...
  const calculateProgress = () => {
    ...
    CATEGORIES.forEach(cat => {
      // Skip smithing if toggled off
      if (cat.id === 'smithing' && !includeSmithingProgression) return;
          ...
          <div className="space-y-4">
            <div className="flex flex-col md:flex-row gap-3">
                ...
                <div className="relative flex-1">
                    ...
                    <input
                        type="text"
                        placeholder="Search items, enchants, trims..."
                ...
                {/* Toggle Progression button */}
                <button 
                    onClick={() => setIncludeSmithingProgression(!includeSmithingProgression)}
                    className={`flex items-center gap-2 px-4 py-2 rounded-lg border text-xs font-bold transition-all ${
                        includeSmithingProgression 
                        ? 'bg-emerald-500/10 border-emerald-500/40 text-emerald-400' 
                        : 'bg-zinc-800 border-zinc-700 text-zinc-500'
                    }`}
                >
                    <Layers size={14} />
                    <span>Smithing Progression: {includeSmithingProgression ? 'ON' : 'OFF'}</span>
                    {includeSmithingProgression ? <ToggleRight size={18} /> : <ToggleLeft size={18} />}
                </button>
            </div>
